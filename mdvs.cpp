...
string getStrFromG1Vec(vector<G1> vec){
    string s = "";
    for (G1 el: vec){
        s += el.getStr();
    }
...
tuple<vector<G1>, G2> sign(G1 g, string msg, vector<G1> public_keys, 
...
void verify (G1 g, string msg, vector<G1> public_keys, 
                Fr sk, int p_id, int v_id, vector<G1> R, G2 S){
    G2 g_hat;
    hashAndMapToG2(g_hat, msg + getStrFromG1Vec(R) + getStrFromG1Vec(public_keys));
    Fr sk_inv;
    Fr::inv(sk_inv, sk);
